...
public class BlockUserController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Perform the block/unblock operation using the user ID
        DAO dao = new DAO();
        boolean success = dao.blockUser(userId);

        // Set the appropriate response status and message
        if (success) {
            response.setStatus(HttpServletResponse.SC_OK);
            response.getWriter().write("User blocked/unblocked successfully.");
        } else {
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            response.getWriter().write("Failed to block/unblock the user.");
        }
